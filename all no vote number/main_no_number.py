...
def open_voting_window():
    ...
    def show_political_parties():
        ...
        parties = {"Andhra Pradesh": ["Telugu Desam Party (TDP)", "YSR Congress Party (YSRCP)", "Bharatiya Janata Party (BJP)", "Indian National Congress (INC)"],
    ...
    def vote():
        ...
        # Create a DataFrame for the new vote result
        new_vote_result = pd.DataFrame({'State': [selected_state], 'Party': [selected_party]})
    
        try:
            # Load existing vote results from Excel
            existing_results = pd.read_excel('vote_results.xlsx')
        
            # Append the new vote result to the existing results
            all_results = pd.concat([existing_results, new_vote_result], ignore_index=True)
        except FileNotFoundError:
            # If the file doesn't exist, create it with the new vote result
            all_results = new_vote_result
